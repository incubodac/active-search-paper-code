...
function jk2022_Unfold_vFun(RR)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [runpath,code_path,session_path,~]=add_paths_EEG_ET_beta(0,'J');% edited by jek 10/11/22
    ...
    fprintf('****************************************************************\n');
    if 1
        run_step1(fp,folderPrepared,inFolder,hipothesis,auxdir,fixs_csv_file, names,subj_index,freqFilteredEEG)
    end
    run_step2(fp,folderOut,folderPrepared,filter_faces,filter_EX,filter_AS,sanity_checks,hipothesis,condi,model)
    run_step3(folderOut,variablesOI)
    run_step4(fp,folderOut,freqFilteredEEG)
end
%% 1st STEP Create EEG with feature information embeded into EEGLAB struct
% This only needs to be run once (for each CSV)
function [] = run_step1(fp,folderPrepared,inFolder,hipothesis,auxdir,fixs_csv_file, names,subj_index,freqFilteredEEG)
    %%%%%%%%%%%%%%%%
    epochedByFixs = 0;
...
function [] = run_step2(fp,folderOut,folderPrepared,filter_faces,filter_EX,filter_AS,sanity_checks,hipothesis,condi,model)
...
function [] = run_step3(folderOut,variablesOI)
    if exist(folderOut, 'dir')
        load(fullfile(folderOut, 'Results.mat')); % unfoldResults
        variableStruct =[];
...
%% 4th STEP run TFCE
function [] = run_step4(fp,folderOut,baselineCorr,freqFilteredEEG)
%     chanlocs = load(fullfile(session_path.aux_data,'locationFile.mat'));
    ...
    load(fileStruct) % variableStruct
...
function tfceResult  = fun_runTFCE(fp,folder,variableStruct,variable,chanlocs)
    ...
    for iSuj = 1:length(variableStruct)
       data  = variableStruct(iSuj).(TFCEvars{ismember(TFCEvars,variable)});
...
function variableStruct = fun_applyBaselineCorrection(variableStruct,variable)
    TFCEvars = fieldnames(variableStruct);    
    if strcmp(variable,'(Intercept)')
        variable = 'Intercept';
    end
    ...
    for iSuj = 1:numel(variableStruct)
          ...
          variableStruct(iSuj).(TFCEvars{var}) = x1 - base1;
    end
end
