...
varcfg = [];
varcfg.Intercept.name       = '(Intercept)';
varcfg.VSNT_EX.name         = 'Task';
varcfg.faces.name           = 'Category';
varcfg.NCRank.name          = 'Rank';
varcfg.NCRank_VSNT_EX.name  = 'Task:Rank';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% limits of significant time-window defined by visual inspection
varcfg.Intercept.xmin_sig   = 0.0;
varcfg.VSNT_EX.xmin_sig     = 0.080;
varcfg.faces.xmin_sig       = 0.120;
varcfg.NCRank.xmin_sig      = 0.050;
varcfg.NCRank_VSNT_EX.xmin_sig= 0.140;

varcfg.Intercept.xmax_sig   = 0.400;
varcfg.VSNT_EX.xmax_sig     = 0.400;
varcfg.faces.xmax_sig       = 0.350;
varcfg.NCRank.xmax_sig      = 0.400;
varcfg.NCRank_VSNT_EX.xmax_sig= 0.400;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for ii = 1:length(variables)
    ...
    figure(10 + ii); clf;
        ...
        title(varcfg.(variable2TFCE).name,'fontsize',42)
        ...
        set(gca,'linewidth', 5)
        set(gcf,'Color','w')
        set(gcf,'Position',[73 1 1848 961])
...
for ii = 1:length(variables)
    variable2TFCE   = variables{ii};
    time_window = [varcfg.(variable2TFCE).xmin_sig varcfg.(variable2TFCE).xmax_sig];
    fileStruct  = fullfile(folderOut,'variableStruct.mat');
    ...
    figure; 
        set(gcf,'Color','w')
        title(varcfg.(variable2TFCE).name)
        topoplot(y,chanlocs,'maplimits',[0 1]); 
...
% Poster plot VSS
% figure(); imagesc(1:64,times,Y([1:32 64:-1:33],:)',[0 0.05]); colormap hot; colorbar
figure(); imagesc(times,1:64,Y([1:32 64:-1:33],:),[0 0.05]); colormap hot; c=colorbar; ylabel(c,'p value','fontsize',42); set(c,'fontsize',35)
